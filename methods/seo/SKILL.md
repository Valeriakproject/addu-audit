...
## Правила ADDU — читать первыми, они главнее методики ниже

Методика ниже — это общая экспертиза по SEO. Порядок работы в агентстве задают эти правила и
`AGENTS.md`. Где расходятся — действуют наши.

### Обязательный лист сбора

SEO-аудит делается **по всему сайту, а не по выборке страниц**. Отчёт по пяти страницам — это не
аудит сайта, даже если в нём честно написано, что страниц было пять. Клиент читает документ как
полный список: чего в нём нет, того, по его мнению, на сайте нет.

До первой строчки отчёта собрать и разобрать:

1. **robots.txt и sitemap.xml.** Выгрузить все адреса из карты сайта в список.
2. **Скачать каждый адрес из этого списка** и по каждому вытащить: код ответа, `title`,
   `description`, `canonical`, `meta robots`, наличие и количество `H1`, объём текста, число
   картинок без `alt`, наличие JSON-LD. 200 страниц скачиваются за пару минут скриптом — руками
   это делать не нужно и нельзя.
3. **Посчитать по этой выгрузке:** сколько страниц под `noindex`, сколько без `title`, без
   `description`, с одинаковым `description`, без `H1`, сколько отдают не 200.
4. **Построить граф внутренних ссылок** по скачанным страницам: сколько входящих ссылок у каждой
   страницы, есть ли сироты, на какой глубине от главной. **Обязательно учитывать
   `<base href>`** — без него все относительные ссылки посчитаются неправильно, и получится
   ложная находка «меню ведёт на 404».
5. **Замерить вес страницы честно:** скачать все подключаемые файлы главной и одной внутренней и
   сложить размеры. «Страница тяжёлая» без килобайтов в отчёт не идёт.
6. **Открыть сайт в браузере на ширине 375–390 px:** горизонтальная прокрутка, зум, размер
   кликабельных элементов, перекрывает ли cookie-баннер экран.
7. **Проверить поддомены.** Список берётся из публичных журналов SSL-сертификатов
   (`https://crt.sh/?q=%25.домен.ru&output=json`). По каждому живому — код ответа, robots.txt,
   `meta robots`. Клиентские копии и внутренние системы, открытые для индексации, — это находка.
8. **Проверить дубли адресов:** `/index.php`, адрес с большой буквы, без слэша в конце, с
   `?utm_source=`, с параметром пагинации. Что отдаёт сервер — 200, 301 или canonical.
9. **Заголовки ответа сервера:** кэширование HTML, сжатие, HTTP/2, HSTS, security-заголовки.
   Это один `curl`, формулировка «требует отдельной серверной проверки» запрещена.

Пропустил что-то из этого списка — пиши в отчёте строкой «не проверено» и говори почему.
Молча пропустить нельзя.

### Что проверить нельзя без доступов

`site:домен.ru` в Яндексе упирается в капчу — **капчу не обходить**. Реальную индексацию,
позиции, трафик и полевые Core Web Vitals без Вебмастера, Search Console и Метрики не получить.
Это идёт отдельным списком «не проверено», а не выдумывается и не заменяется оценкой.
PageSpeed Insights может ответить отказом по квоте — тогда в отчёте только собственные замеры,
и об этом сказано прямо.

### Каждая цифра проверяется дважды

Перед тем как написать число, вернуться к выгрузке и пересчитать. Длина `description` берётся
измерением, а не на глаз. Если в одном месте отчёта написано «22 технических и noindex URL», а
`noindex` стоит на 77 страницах — отчёт неверный, даже если остальное правда.

Отдельно следить за счётом записей и уникальных адресов: в карте сайта бывают задвоенные
строки, и одна и та же страница может посчитаться дважды.

### Формат выдачи — файл, не ссылка

**SEO-аудит клиенту уходит PDF-файлом. На case.addu.ru не публикуется** (решение Леры от
18.09.2026). Причина: в SEO-отчёт попадают открытые служебные разделы, тестовые копии и
поддомены клиента — публиковать такое в интернете нельзя, даже по ссылке без индексации.

Отчёт делать **линейным документом, без вкладок** — вкладки не печатаются в PDF. В CSS нужен
`print-color-adjust: exact`, иначе PDF уходит без фона. Перед отправкой PDF пролистать глазами и
сверить с веб-версией.

### Находка про утечку данных

Если на сайте нашёлся открытый служебный раздел с чужими персональными данными (журналы форм,
выгрузки заявок, логи): **в отчёте не писать точный адрес и не воспроизводить сами данные**.
Описать суть, отдать адрес отдельно — ответственному за устранение. В отчёте это находка
номер один, и до её закрытия остальные работы не имеют смысла.

### Смета

Ставка 2 500 ₽/час, часы вверх до 0,5 ч, клиенту одна фиксированная сумма. Скидка 30% на
SEO-аудит **не распространяется** — она только для 152-ФЗ. Сверить четыре числа: часы на
обложке, сумма часов по строкам, сумма стоимостей, итог. Оформление — `assets/report-template.html`,
блок «Контакт по проекту» обязателен.

Перед сдачей прогнать `qa-review-rules.md` из этой же папки.
